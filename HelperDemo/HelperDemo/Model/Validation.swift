...
func validataEmail(strEmail: String?) -> Bool {
...
func validataPhoneNumber(strPhone: String?) -> Bool {
    
    let phoneRegex = "(([+]{1}?)?([0-9]{10}))" //  (([+]{1}?)?(\d{10}))  //"(([+0-9]{11})|([0-9]{10}))"
    let phonePredicate = NSPredicate(format: "SELF MATCHES %@",phoneRegex)
    return phonePredicate.evaluate(with: strPhone!)
}

func validataPasswordLength(strPassword: String?) -> Bool {
    
    // Simple Password regex
    let passwordRegex = "([A-Za-z0-9!@#$%^&*,.;':?/\\|]{6,}$)"
    let passwordPredicate = NSPredicate(format: "SELF MATCHES %@",passwordRegex)
    return passwordPredicate.evaluate(with: strPassword!)
}
